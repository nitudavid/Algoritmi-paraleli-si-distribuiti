...
void *rescale_image(void *arg) {
	...
	if (data->scaled_image == NULL) {
		pthread_exit(NULL);
	}

	int start = data->thread_id * (data->scaled_image->x / NUM_THREADS);
    int end = (data->thread_id + 1) * data->scaled_image->x / NUM_THREADS;
    if(end > data->scaled_image->x) {
        end = data->scaled_image->x;
    }
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < NUM_THREADS; i++) {
        ...
        thread_data[i].original_image = image;
		thread_data[i].scaled_image = new_image;
        thread_data[i].step_x = step_x;
        thread_data[i].step_y = step_y;

        pthread_create(&threads[i], NULL, rescale_image, &thread_data[i]);
    }
	...
	ppm_image *scaled_image = thread_data[0].scaled_image;
    if(scaled_image == NULL) {
        scaled_image = thread_data[0].original_image;
    }
